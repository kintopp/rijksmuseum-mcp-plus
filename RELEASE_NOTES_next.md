...
- **Many more ways to break down the collection.** The collection-statistics view can now
  ...
  profession, gender, or birth decade or century, by where a maker was born or died, by the
  kind of place where a work was made (a city, a region, a country), and by the role a party
  held in an ownership history — collector, buyer, heir, donor, and so on.
...
- **Withdrawn works show up when reviewing recent changes.** A listing of recently changed records
  now marks works that have been removed from the catalogue as deletions, alongside those added or
  revised, so a record's withdrawal can be seen rather than inferred from a silent absence.
...
deployed at time of writing); ships with the next code push, no DB swap needed:
  - collection_stats: 9 new dimensions + many new filters — productionRole / profession /
    ...
    cross-ref event flags. The placeType dimension's bucket labels — originally raw AAT/Wikidata
    URIs, the reason it was first held out of the body — are now resolved to human labels and the
    filter accepts a label or a URI, so placeType is now folded into the "ways to break down" bullet.
    The redundant dimensionCoverageCount field was dropped before release.
                                       [#320 eccaf9d; placeType plan 027 5bda51b / plan 028 eab6916;
                                        dimensionCoverageCount drop #348 plan 026 501a27e]
...
get_recent_changes — deletions surfaced. OAI-PMH deleted records (header status="deleted", with
deletedRecord=persistent) are now reported with deleted:true / [DELETED] (lodUri fallback when the
objectNumber is absent); previously dropped. Additive output (the records schema is open — no schema
change). Runtime-only code, merged to main (not yet pushed/deployed); ships with the next code push,
no DB swap. Live deletions are near-zero, so coverage is via a fixture test.        [#394, merge 96ff78b]
...
Follow-up filed (not in this release): EDM-shape breaking-change watch — the museum announced an
OAI-PMH release making EDM Europeana-schema-valid (notice 2026-06-11); a current-shape baseline was
captured (scripts/probe-edm-shape.py) to diff against. Dev tooling only, no user-facing change. [#406]
...
TODO at release time:
  ...
  - Confirm the runtime collection-exploration + search features ([#320] / [#356] / [#393] /
    [#381]) were pushed/deployed; they need only a code push, not a DB swap. Includes the
    placeType label resolution ([#320] plan 027/028) behind the "ways to break down" bullet
    and the get_recent_changes deletions ([#394]) behind the "withdrawn works" bullet — both
    code push only, no DB swap; drop those two bullets if the push hasn't shipped at release.
