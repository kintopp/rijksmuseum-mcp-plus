...
When searching, matching is accent-insensitive (so "Eugene Brands" finds "Eugène Brands") and relevance-ranked. The museum's Search API always returns results in catalogue order. In practice that means its first search results simply list whatever was catalogued earliest rather than whatever best matches the query — so an AI assistant (or person) who reads only the top few hits could miss the most relevant work entirely.
