...
The easiest way to try rijksmuseum-mcp+ is with [Claude Desktop](https://claude.com/download) or [claude.ai](https://claude.ai) and the hosted version of the MCP server using the URL below. Note that this currently requires a [subscription](https://claude.com/pricing) from Anthropic. 
```
...
The rijksmuseum-mcp+ MCP server is also compatible with open-source LLM client applications, such as [Jan.ai](https://jan.ai) which do not require a monthly, paid subscription. Instead, configure the app to use a cheap, usage based API key from a LLM provider (e.g. from Anthropic or OpenRouter).
...
| `narrative` | Curatorial narrative (museum wall text).  | Full-text search across ~14K artworks with curatorial narratives. Harvested from the Linked Art `subject_of` field — distinct from the Search API's `description` parameter which queries a different, broader text. These are interpretive, art-historical texts written by museum curators — equivalent to the wall labels in the galleries. |
...
| Curatorial narrative (`curatorialNarrative`) | Museum wall text in English and/or Dutch.  | Interpretive art-historical context written by curators. Harvested from the Linked Art `subject_of` field. Distinct from `description` (Search API). Searchable via the `narrative` filter above (~14K artworks), but the full text is only returned here. |
