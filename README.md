...
Recent ([v0.19](https://github.com/kintopp/rijksmuseum-mcp-plus/releases/tag/v0.19.0)):

- Added relevance ranking for text-based searches (title, description, inscription, narrative, provenance).
- Added Importance ordering for vocabulary-filter queries
- With ranking in place, reduced default/max results: 25/50 for search, 15/50 for semantic search. Better for LLMs.
- LLMs now get counts per facet on truncated results to guide new filter selections
