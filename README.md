...
[![MCP Apps](https://img.shields.io/badge/MCP_Apps-v1.0.1-teal)](https://github.com/modelcontextprotocol/ext-apps)
...
- ~~Relevance ranking for text-based searches (title, description, inscription, narrative, provenance). Importance ordering for vocabulary-filter queries~~
- ~~With ranking in place, reduce default/max results: 25/50 for search, 15/50 for semantic search. Better for LLMs.~~
- ~~LLM should get counts per facet on truncated results to guide filter selections~~
- ~~Add morphological stemming for English subject queries (else subject: 'cat' ≠ 'cats')~~
- ~~Add attribution filter (attributed to, workshop of, etc.)~~
- ~~Use Rijksmuseum search API only as fallback~~
...
- investigate improved image viewer functionality
- investigate means to export image viewer image + overlays
