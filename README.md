...
4. **More metadata** — several metadata fields not searchable from the museum's [search portal](https://www.rijksmuseum.nl/en/collection) (`birthPlace` / `deathPlace`, `profession`, `iconclass`, `collectionSet`, `attributionQualifier` — e.g. "workshop of", "attributed to", "circle of") as well as creator demographics (`gender`, `birth/death years`, `biographical notes`) and place hierarchy expansion for spatial searches.
