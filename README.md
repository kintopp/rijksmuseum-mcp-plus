...
|---|---|---|
| `query` | Artwork title (broad match) | ~837K artworks. Maps to the API's `title` parameter. Less targeted than the `title` filter below; useful for exploratory queries where the exact title wording is unknown. |
| `title` | Artwork title (exact field match) | ~826K artworks with titles. Matches against the Linked Art `identified_by` field classified as [AAT 300417207](http://vocab.getty.edu/aat/300417207) (title). Takes precedence over `query` when both are supplied. |
