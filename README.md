...
When searching, matching is accent-insensitive (so "Eugene Brands" finds "Eugène Brands") and relevance-ranked. The museum's Search API returns results in a fixed order (by internal identifier), not by relevance — so its first hits are not necessarily the best matches, and an AI assistant (or person) who reads only the top few could miss the most relevant work entirely.
