...
## Step 3: Batch price flag — REQUIRED after Step 0 (corrected 2026-06-14)

**Run this after Step 0.** Earlier guidance here said "REDUNDANT after Step 0" — that was
**wrong** and is corrected. The PEG parser sets some `batch_price` during reparse via
`BATCH_PRICE_RE` (`src/provenance-peg.ts:29`), but that regex is **narrower** than the
`LIKE` patterns below. In the 2026-06-14 prerequisite-#1 re-parse the parser left **596
priced batch-price events unflagged** (e.g. "…with other objects, fl. 2,000…", "…six other
objects…") that this SQL correctly flags. Skipping it drops `batch_price + price_amount`
**below band** (3314 vs target 3700–4100). The SQL sets the flag only — no price/data values
change — and is idempotent (re-setting already-flagged rows is a no-op; ~3,781 rows touched,
~596 net new flags).
...
--     Tail party "to the museum" the rule-based parser did not capture.
--     STALE COMMENT (2026-06-14): this INSERT is now a NO-OP. After the #185/Option-B
--     relabel, writeback-missing-receivers (Step 2b) DOES capture this "to the museum"
--     tail and labels it position_method='rule:missing_receiver' — so the INSERT OR IGNORE
--     below finds an existing (artwork,seq,party_idx) row and changes nothing (verified:
--     changes=0 in the prerequisite-#1 re-parse). That is CORRECT under Option B: these
--     rule-based receivers are intentionally §G-excluded and regenerated every re-parse,
--     not stored. The 'llm_enrichment' intent below no longer applies; left for the record.
...
# 7d. Party extraction: parties for events with 0 extracted parties (#116 edge cases)
#     correction_method 'llm_structural:missing_all_parties' / 'missing_sender'.
#     CRITICAL (2026-06-14): the 7d party-extraction audit JSON was NEVER persisted to the
#     repo — the prior runs were on-demand API batches that were applied and discarded. The
#     PREVIOUS deployed DB is the only surviving record. So the REPRODUCIBLE path is to
#     reconstruct from that DB (NOT regenerate via the API — that is non-deterministic and
#     won't match the deployed state). In the v0.40 baseline this is 199 events / 361 parties
#     (all position_method='llm_structural'):
node scripts/reconstruct-7d-from-baseline.mjs --base <previous-deploy>.db --apply
#     (dry-run first without --apply; it is raw_text-guarded and skips any re-segmented event).
#
