...
The 16 rows were identified by a TGN RDF re-validation dry-run on 2026-05-09
(`scripts/geocoding/batch_geocode.py --revalidate-tgn-rdf`) — every one of
them 404s on both the .rdf endpoint and the human-facing TGN HTML page.

Patterns:
  1. Iconclass notation routed to TGN (2 rows): id='25H214' on two places.
  2. 9-digit numbers (5 rows) — far outside the real 6-8 digit TGN range.
  3. 7-digit numbers in the plausible range, but never in TGN (9 rows).
     Notable: Abu Simbel here is tagged '7520358'; TGN's real ID is 7000079.
     This script doesn't repair such cases — it only removes the wrong row;
     the place itself stays in `vocabulary`.

The companion classifier hardening in `scripts/harvest-vocabulary-db.py`
(`_AUTHORITY_ID_PATTERNS`, #335) prevents future Pattern-1 leaks but cannot
catch Patterns 2 and 3 at parse time — those pass any charset/length guard
and only fail at TGN-lookup time. The TGN RDF re-validation pass is the
detection net for the latter; this script is the matching cleanup.

Idempotent: only deletes the listed (vocab_id, id) tuples, re-running
on an already-clean DB updates 0 rows.
...
# (vocab_id, id) tuples from the #335 TGN RDF dry-run, 2026-05-09.
BOGUS_ROWS: list[tuple[int, str]] = [
    # Pattern 1: Iconclass notation in TGN URI
    (23022333, "25H214"),     # "Prospect Lake"
    (23022490, "25H214"),     # "Hickling Broad"
    # Pattern 2: 9-digit numbers (~100M-700M), outside TGN's range
    ...
    # Pattern 3: 7-digit numbers in the plausible TGN range but never in TGN
...
def main() -> int:
    ...
    for vocab_id, ext_id in BOGUS_ROWS:
        ...
        if row is None:
            missing.append((vocab_id, ext_id))
        else:
