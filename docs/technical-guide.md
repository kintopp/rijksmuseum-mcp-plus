...
| Tool | Description |
|---|---|
| `search_artwork` | Search the collection by creator, type, material, technique, date, title, description, and [37 search filters](search-parameters.md) including subject, iconclass, depicted person/place, production place, provenance, inscription, curatorialNarrative, dimensions, geo proximity, creator demographics (`creatorGender`, `creatorBornAfter`/`creatorBornBefore`), and place hierarchy expansion (`expandPlaceHierarchy`). Returns up to 25 results (max 50). Compact mode and pagination supported. |
