...
> **Proposed capability ([#363](https://github.com/kintopp/rijksmuseum-mcp-plus-offline/issues/363)), not yet shipped.** The scenarios below describe a planned `textQuery` parameter that would expose structured boolean, phrase, proximity, and prefix search over the catalogue's free-text fields. Unlike the scenarios above, they have no worked share links — they document what the feature would unlock and why the existing flat filters cannot reach it.

The free-text filters available today — `title`, `description`, `inscription`, `curatorialNarrative` — are flat and AND-combined: each matches a single column, and a user's words are treated as one literal phrase. That is a deliberate safety choice, but it means a researcher cannot express boolean logic, control phrase-versus-token matching, require two terms in proximity, match a word-stem with a wildcard, or combine fields with *either/or*. A structured `textQuery` would compile these into safe full-text queries server-side, on the same indexes already shipped — no new infrastructure. The three scenarios below each exercise a different part of that query surface, grounded in the actual language of the catalogue's three long-form corpora: the **Dutch, denotative** `description` field (~512K works), the **mostly-English, interpretive** `curatorialNarrative` field (~14K works), and the **literal transcribed** `inscription` field (~502K works).

### 26. A Theme Split Across the Bilingual Catalogue

*Where in the collection does the Beeldenstorm — the 1566 wave of iconoclasm — surface in the catalogue's own words, whether a Dutch cataloguer recorded it as "beeldenstorm" in the object description or an English curator framed it as "iconoclasm" in the wall text? Exclude the later history prints that merely depict the event as a subject.*
...
**Why it matters:** A single concept is lexicalised *differently in each corpus and each language*. `description: "beeldenstorm"` and `curatorialNarrative: "iconoclasm"` return almost entirely different works, and the two sets barely overlap. The flat filters can only AND the two columns together — yielding the near-empty *intersection* — never the *union*. Cross-column OR is the only way to ask "this idea, however the cataloguer happened to phrase it, in whichever language they wrote." Collapsing the clauses into one match also means the relevance ranking finally spans the whole text query rather than just the first field.

### 27. Securely Signed *and* Dated Works as an Authentication Cohort
...
**Why it matters:** A description that mentions a signature *somewhere* and a date *somewhere else* is not the same as one asserting the two together. Requiring `gesigneerd` and `gedateerd` to sit close drops the loose co-occurrences and keeps the works the cataloguer described as signed-and-dated in one breath — a methodological filter for authentication studies. Neither the vocabulary filters nor `semantic_search` can enforce token adjacency; proximity has no substitute elsewhere in the toolset.

### 28. The Division of Labour on the Printed Plate

*Dutch and Flemish prints record their production chain directly on the plate, in Latin: who designed the composition (`invenit`/`inventor`), who engraved it (`sculpsit`/`fecit`/`incidit`), and who published it (`excudit`). Find prints whose inscription documents a full three-role chain — and surface the cases where one person did two jobs, such as the engraver who was also his own publisher.*
...
**Why it matters:** The same role appears on the plate in many forms, because engravers abbreviated to fit the margin and switched between verb and agent-noun. Prefix matching unifies them: an exact search for `"sculpsit"` finds a fraction of the works that `sculp*` does, once `sculptor`, `Sculpt.` and `sculp.` are folded in. The `productionRole` filter in `search_artwork` normalises these relationships into a small set of curated English labels and is mapped only where a cataloguer resolved the role to a named agent — so it cannot preserve the literal inscribed Latin, detect a *role combination on a single object* (the `sculptor et excudit` double-duty), or reach prints where the plate inscription is the only record. The inscription is the primary source; the structured role is a partial derivation of it, and a prefix-and-boolean query reads the source directly. The trade-off is precision: prefix widens recall (`inven*` also catches stray `inventaris` notes), so the multi-clause AND — requiring an engrave-role and a publish-role alongside the design-role — is what keeps the cohort clean without hand-curation.
