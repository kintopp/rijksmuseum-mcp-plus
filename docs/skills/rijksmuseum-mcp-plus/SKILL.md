...
When the user wants a region examined, an inscription or signature read, or a labelled overlay placed on the open viewer, follow the pixel-geometry recipe: check `get_artwork_details`' `description`/`curatorialNarrative` first for a catalogued location, survey with `region:"full"`, then `inspect_artwork_image` at a tight `pct:` crop → place the overlay in crop-local `crop_pixels:` via `navigate_viewer`'s `relativeTo`/`relativeToSize` → verify with `show_overlays`. Two tools, different audiences — `get_artwork_image` opens the viewer for the **user**; `inspect_artwork_image` returns bytes for the **model** (and auto-zooms the open viewer to whatever you inspect). **Honesty caveat:** figure-level semantic highlights (a face, a gesture, a specific object among several) are approximate — narrate them as a checkable claim ("I've boxed what I identified as X — please confirm") rather than an assertion; region zoom, catalogued-location pointing, and user-driven highlighting remain reliable. Full recipe — catalogue-first localization, survey-before-drilling, edge-snapping, magnify-before-measuring, one-overlay-per-instance, and verification — in [`references/specialist-workflows.md`](references/specialist-workflows.md#5-image-inspection-and-overlay-placement).
