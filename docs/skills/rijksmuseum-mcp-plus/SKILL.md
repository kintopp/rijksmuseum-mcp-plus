...
| `search_persons` returns 0 rows for a demographic filter | The `gender`, `bornAfter`, `bornBefore` filters require person enrichment to be present on the vocab DB. Only on a fresh harvest without enrichment do they return zero rows — the reference deployment is enriched and these filters work there. Name-token matching and structural filters (`birthPlace`, `deathPlace`, `profession`) work regardless. |
