...
- `get_artwork_details` → `attributionMarks` reports the *count* of signature/inscription marks on a work (presence only — the harvested rows carry no transcribed text and their carrier URIs do not resolve); use `parsedInscriptions` / `search_inscriptions` for what is actually written. A zero count does not imply the work is unsigned: the mark rows and the inscription field are independent datasets (The Night Watch returns `attributionMarks: 0` yet carries "Rembrandt f 1642" in `parsedInscriptions`), so always check `parsedInscriptions` before reporting absence of signature evidence. For the full forensics record (technical examinations + restoration treatments + that mark count + a provenance excerpt), use `get_conservation_history`.
- `get_conservation_history` covers only a **small fraction of a percent** of the collection, and famous paintings are not preferentially included — the Night Watch and *The Milkmaid* both return nothing despite decades of published treatment. An empty result means "no examination record was harvested for this work", never "this work was never examined"; say so rather than implying the museum has no conservation data.
...
collection_stats(dimension="creator", collectionSet="Japan (collectie)", topN=20)
# → top 20 creators in the Japan set. Set names are Dutch: discover them with
# list_curated_sets, never by guessing an English name (an unmatched name
# returns zero artworks with a "no vocabulary matches" warning).
...
| Multi-folio works dominate results (sketchbooks, albums, print series) | The Rijksmuseum catalogues sketchbooks/albums/print-series as a parent record plus child records per folio, so a single physical object can fill the first page of results. Use `groupBy: "parent"` on `search_artwork` to collapse children whose parent is also in the result set (the parent gains a `groupedChildCount`). When `groupBy` isn't set, the response's `warnings` field flags clustering (e.g. "8 results are folios/components of BI-1898-1748A"). Affects roughly one in twenty artworks (object numbers with parenthetical suffixes). |
| `nearPlace` with a small radius returns a whole city | Many place-entry coordinates are minutes-precision (~2 km granularity), so a landmark and its city can share the same rounded centroid — a 0.1 km radius around "Oude Kerk Amsterdam" matches the entire "Amsterdam" vocab entry at distance 0. Matching targets place-entry centroids, so radii below that precision cannot isolate a landmark from its city, whichever way the query point is given (`nearPlace` or precise `nearLat`/`nearLon`). Check `referencePlace`, each result's `nearestPlace`/`distance_km`, and the disambiguation warning; treat sub-2 km results as city-level. |
