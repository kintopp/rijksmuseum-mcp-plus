...
| 16 | **Extent text** | `extentText` | Verbose human-readable extent/dimensions string (`dcterms:extent`), distinct from the structured `dimensions` and the reconstructed `physicalDimensions`. Also surfaced (under the same `extentText` key) by `browse_set` records. |
...
| 28 | **License** | `license` | Rights/license URI (e.g. CC0 1.0, Public Domain Mark) from the `rights_lookup` table. Not a standalone search filter; returned on each result, and available as the `rights` dimension via `search_artwork`'s `facets` parameter (e.g. `facets: ["rights"]`) for a license breakdown of a result set. |
...
| 30 | **Parents** | `parents` | Parent records (e.g. the sketchbook this folio belongs to). Each entry: `objectNumber`, `title`. Empty for top-level objects. |
| 31 | **Children** | `children` / `childCount` | Child records (e.g. folios in a sketchbook, leaves in an album). `children` returns up to 25 entries ordered by object number; `childCount` reports the total. Use `search_artwork` to enumerate the full set. |

## Audit Timestamps

| # | Category | Field | Description |
|---|----------|-------|-------------|
| 32 | **Record created** | `recordCreated` | ISO 8601 timestamp of catalogue record creation. |
| 33 | **Record modified** | `recordModified` | ISO 8601 timestamp of the catalogue record's most recent modification. Not a range filter; order a result set by it with `search_artwork`'s `sort: 'recordModified:desc'`, or use `get_recent_changes` to list recently modified records. |
