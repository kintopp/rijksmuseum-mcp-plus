...
export const PRIORITY_LEVEL_QUALIFIER_URIS: readonly string[] = [
...
// Same-row intercepts (#349/#357) hit these three filters per query. Caching the
// VocabFilter references avoids a linear VOCAB_FILTERS.find() scan on every search.
const CREATOR_FILTER = VOCAB_FILTERS.find(f => f.param === "creator")!;
...
function emitSameRowExists(
  ...
  creatorIdsPerElement: readonly string[][],
  secondaryIdsPerElement: readonly string[][],
  conditions: string[],
  bindings: unknown[],
): void {
...
export class VocabularyDb {
  ...
  // ── find_similar (cluster extracted to VocabularyDb.similarity.ts) ──
  // The 7 channels' lazy IDF caches + the mappings-lookup statement now live in
  // SimilarityQueries. Only the constructor-time flags + qualifier-id sets that
  // are ALSO read by the attribution/production-role paths remain here.
  ...
  // deliberately NOT a find_similar Related Variant label (treated as companions, not similar artworks).
  private static readonly RELATED_PHYSICAL_LABELS = [
    "object | current frame", "object | former frame", "object | pedestal",
  ] as const;
  /** find_similar similarity cluster (src/api/VocabularyDb.similarity.ts). Constructed in the
   *  constructor once the shared statements are prepared; undefined when no DB is loaded. */
  private similarity: SimilarityQueries | undefined;
  ...
  constructor() {
    ...
    try {
      ...
      // #349: caches for same-row matching against assignment_pairs. The qualifier-IDs
      // set is queried straight from the table (gives us the connoisseurship codes
      // present in the data); the priority-level set is resolved from fixed AAT URIs
      // so we can distinguish "user supplied a real priority qualifier" from
      // "label FTS happened to match an Iconclass entry" when emitting warnings.
      if (this.hasAssignmentPairs) {
        const qualRows = this.db.prepare(
          "SELECT DISTINCT qualifier_id FROM assignment_pairs"
        ).all() as { qualifier_id: string }[];
        this.assignmentPairsQualifierIds = new Set(qualRows.map(r => r.qualifier_id));

        const priorityPlaceholders = PRIORITY_LEVEL_QUALIFIER_URIS.map(() => "?").join(", ");
        const priorityRows = this.db.prepare(
          `SELECT id FROM vocabulary WHERE external_id IN (${priorityPlaceholders})`
        ).all(...PRIORITY_LEVEL_QUALIFIER_URIS) as { id: string }[];
        this.priorityLevelQualifierIds = new Set(priorityRows.map(r => r.id));
      }
      ...
      // find_similar cluster — reaches this class only through these injected deps.
      this.similarity = new SimilarityQueries({
        db: this.db!,
        hasAssignmentPairs: this.hasAssignmentPairs,
        hasRelatedObjects: this.hasRelatedObjects_,
        requireFieldId: (name) => this.requireFieldId(name),
        hasField: (name) => this.fieldIdMap.has(name),
        lookupArtRow: (objectNumber) =>
          this.stmtLookupArtId!.get(objectNumber) as
            { art_id: number; title: string; creator_label: string } | undefined,
        batchLookupByArtId: (ids) => this.batchLookupByArtId(ids),
        batchLookupTypesByArtId: (ids) => this.batchLookupTypesByArtId(ids),
        batchLookupImportanceByArtId: (ids) => this.batchLookupImportanceByArtId(ids),
      });
      ...
      if (this.hasRelatedObjects_) {
        const relatedVariantPlaceholdersForCount = RELATED_VARIANT_LABELS.map(() => "?").join(", ");
        this.stmtArtworkRelatedCount = this.db.prepare(
          `SELECT COUNT(*) AS n FROM related_objects
             WHERE art_id = ? AND relationship_en IN (${relatedVariantPlaceholdersForCount})`
        );
        // Viewer-side relatedObjects[] is restricted to the same 3 creator-
        // invariant types used by find_similar's Related Variant
        ...
        const relatedVariantPlaceholders = RELATED_VARIANT_LABELS.map(() => "?").join(", ");
  ...
  private fetchRelatedObjects(
    ...
    // Both prepared statements bind the 3 RELATED_VARIANT_LABELS as label
    // filters in addition to artId / limit.
    const labels = RELATED_VARIANT_LABELS;
  ...
  // ── find_similar: delegated to SimilarityQueries (src/api/VocabularyDb.similarity.ts) ──
  // Thin forwarders keep the public method names + the no-DB → null contract
  // (the `?.` short-circuits when `this.similarity` was never constructed).

  findSimilarByIconclass(objectNumber: string, maxResults: number): IconclassSimilarResult | null {
    return this.similarity?.findSimilarByIconclass(objectNumber, maxResults) ?? null;
  }

  findSimilarByLineage(objectNumber: string, maxResults: number): LineageSimilarResult | null {
    return this.similarity?.findSimilarByLineage(objectNumber, maxResults) ?? null;
  }

  findSimilarByDepictedPerson(objectNumber: string, maxResults: number): DepictedSimilarResult | null {
    return this.similarity?.findSimilarByDepictedPerson(objectNumber, maxResults) ?? null;
  }

  findSimilarByDepictedPlace(objectNumber: string, maxResults: number): DepictedSimilarResult | null {
    return this.similarity?.findSimilarByDepictedPlace(objectNumber, maxResults) ?? null;
  }

  findSimilarByTheme(objectNumber: string, maxResults: number): DepictedSimilarResult | null {
    return this.similarity?.findSimilarByTheme(objectNumber, maxResults) ?? null;
  }

  findSimilarByRelatedVariant(objectNumber: string, maxResults: number): DepictedSimilarResult | null {
    return this.similarity?.findSimilarByRelatedVariant(objectNumber, maxResults) ?? null;
  }

  findSimilarByRelatedObject(objectNumber: string, maxResults: number): DepictedSimilarResult | null {
    return this.similarity?.findSimilarByRelatedObject(objectNumber, maxResults) ?? null;
  }
  ...
  private searchInternal(params: VocabSearchParams, compact: boolean, timings?: SearchTimings): VocabSearchResult {
    ...
    // Column names are single-sourced from TEXT_QUERY_FIELD_COLUMNS (db.ts); the
    // order here fixes which flat filter claims the rank JOIN when several are set.
    const TEXT_FILTERS = (["description", "inscription", "curatorialNarrative", "title"] as (keyof VocabSearchParams)[])
      .map((param) => [param, TEXT_QUERY_FIELD_COLUMNS[param as string]] as [keyof VocabSearchParams, string]);
    const requestedTextFilters = TEXT_FILTERS.filter(([param]) => typeof effective[param] === "string");

    // BM25 ranking: one FTS filter claims the rank JOIN slot (textQuery first,
    // else the first flat filter); every remaining flat filter becomes an
    // IN-subquery. The JOIN's MATCH operand is `fts.<column>` — a real column
    // for a flat filter, or the table-named column `fts.artwork_texts_fts` for a
    // textQuery, whose compiled match carries its own `{cols}:` scoping.
    let ftsMatch: { column: string; binding: unknown } | null = null;

    // #363: structured textQuery DSL — compiled into one multi-column MATCH.
    if (effective.textQuery) {
      if (this.hasTextFts) {
        const compiled = compileTextQuery(effective.textQuery);
        if ("error" in compiled) warnings.push(`textQuery ignored: ${compiled.error}.`);
        else ftsMatch = { column: "artwork_texts_fts", binding: compiled.match };
      } else {
    ...
    if (requestedTextFilters.length > 0) {
      if (this.hasTextFts) {
        for (const [param, column] of requestedTextFilters) {
          ...
          if (!ftsMatch) {
            ftsMatch = { column, binding: ftsPhrase };
          } else {
    ...
    // Emit the rank JOIN once, from whichever filter claimed the slot.
    let ftsJoinClause = "";
    let ftsJoinBinding: unknown | null = null;
    let ftsRankOrder = false;
    if (ftsMatch) {
      ftsJoinClause = `JOIN artwork_texts_fts fts ON fts.rowid = a.rowid AND fts.${ftsMatch.column} MATCH ?`;
      ftsJoinBinding = ftsMatch.binding;
      ftsRankOrder = true;
    }
  ...
  private applySameRowIntercepts(
    ...
    const qualifierIntercept =
      qualifierRaw !== undefined && this.hasAssignmentPairs && this.assignmentPairsQualifierIds !== null;
    const roleIntercept =
      sameRowFlag && productionRoleRaw !== undefined && this.hasProductionRolePairs;

    if (!qualifierIntercept && !roleIntercept) {
      // sameRowMatching:true with productionRole but no production_role_pairs table —
      // surface the unmet requirement and let the default loop run.
      if (sameRowFlag && productionRoleRaw !== undefined && !this.hasProductionRolePairs) {
        warnings?.push(
    ...
    const creatorIdsPerElement = creatorValues.map(v => this.resolveVocabTextIds(CREATOR_FILTER, v));
    ...
    if (qualifierIntercept) {
      ...
      const perElement = qualifierValues.map(v => {
        const allIds = this.resolveVocabTextIds(QUALIFIER_FILTER, v);
        return {
          inAP: allIds.filter(id => apSet.has(id)),
          priorityHits: allIds.filter(id => priorityIds.has(id)),
        };
      });

      if (perElement.every(q => q.inAP.length > 0)) {
        emitSameRowExists(
          "assignment_pairs", "ap", "qualifier_id",
          creatorIdsPerElement, perElement.map(q => q.inAP),
          conditions, bindings,
        );
        skipParams.add("creator");
        skipParams.add("attributionQualifier");
      } else if (perElement.some(q => q.inAP.length === 0 && q.priorityHits.length > 0)) {
        warnings?.push(
    ...
    if (roleIntercept) {
      ...
      const roleIdsPerElement = roleValues.map(v => this.resolveVocabTextIds(ROLE_FILTER, v));
      if (roleIdsPerElement.some(r => r.length === 0)) return null;

      emitSameRowExists(
        "production_role_pairs", "prp", "role_id",
        creatorIdsPerElement, roleIdsPerElement,
        conditions, bindings,
      );
      skipParams.add("creator");
